...
exports.syncAmazonDataToShipstation = void 0;
const shipstation_1 = require("../shared/shipstation"); // Use existing functions
 ...
 */
async function syncAmazonDataToShipstation(shipstationOrderId, shipstationOrderNumber, itemsToUpdate, options, orderDebugInfo // Pass the main debug object
) {
    ...
    logger.info(`[ShipstationSync][Order ${orderIdForLog}] Preparing to sync ${Object.keys(itemsToUpdate).length} item(s) with Amazon data to ShipStation Order ID ${ssOrderIdForLog} (Number: ${ssOrderNumForLog}).`);
    ...
    try {
        ...
        if (orderStatusLower === 'shipped' || orderStatusLower === 'fulfilled') {
            logger.warn(`[ShipstationSync][Order ${orderIdForLog}] ⚠️ SKIPPING UPDATE: Order ${ssOrderNumForLog} (ID: ${ssOrderIdForLog}) is already marked as "${ssOrder.orderStatus}" in ShipStation. Updates will likely be ignored.`);
        ...
        if (options.dryRun) {
            ...
            const success = await (0, shipstation_1.updateOrderItemsOptionsBatch)(ssOrder, itemsToUpdate, auditNote);
            if (success) {
...
exports.syncAmazonDataToShipstation = syncAmazonDataToShipstation;
