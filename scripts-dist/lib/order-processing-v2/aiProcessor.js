...
exports.extractPersonalizationWithAI = void 0;
const logger_1 = require("./logger");
const types_1 = require("./types"); // Import Zod schema for validation
/**
 ...
 */
async function extractPersonalizationWithAI(order, options, prisma // Pass prisma instance for logging
) {
    const logger = (0, logger_1.getLogger)();
    const startTime = Date.now();
    const modelUsed = options.openaiModel;
    let rawResponse = null;
    let fullPromptForDebug = null;
    try {
...
`;
        ...
        logger.info(`[AIProcessor][Order ${order.id}] OpenAI call response status: ${response.status} (${duration}ms).`);
        if (!response.ok) {
            const errorBody = await response.text();
            logger.error({ status: response.status, body: errorBody }, `[AIProcessor][Order ${order.id}] OpenAI API error`);
        ...
        const validationResult = types_1.AiOrderResponseSchema.safeParse(responseJson);
        ...
        try {
            const tasksGenerated = Object.values(validatedData.itemPersonalizations).reduce((sum, item) => sum + item.personalizations.length, 0);
            const needsReviewCount = Object.values(validatedData.itemPersonalizations).reduce((sum, item) => sum + (item.overallNeedsReview ? 1 : 0), 0);
            await prisma.aiCallLog.create({
                data: {
                    scriptName: 'populate-print-queue-v2',
                    orderId: order.id,
            ...
            logger.error(`[AIProcessor][Order ${order.id}] Failed to log successful AI call to database: ${logError instanceof Error ? logError.message : String(logError)}`);
        ...
        try {
            ...
            logger.error(`[AIProcessor][Order ${order.id}] Failed to log AI error to database: ${logError instanceof Error ? logError.message : String(logError)}`);
        }
        return {
            success: false,
            error: errorMsg,
...
exports.extractPersonalizationWithAI = extractPersonalizationWithAI;
