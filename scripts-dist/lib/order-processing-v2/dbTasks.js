...
exports.createOrUpdateTasksInTransaction = void 0;
 ...
 */
async function createOrUpdateTasksInTransaction(tx, order, aiData, options, orderDebugInfo // Pass the main debug object
) {
    const logger = (0, logger_1.getLogger)();
    ...
    for (const item of orderInTx.items) {
        ...
        if (!itemDebugEntry) {
            itemDebugEntry = {
                ...
                createdTaskIds: [],
                shipstationUpdatePrepared: false,
        ...
        if (!itemAiResult || itemAiResult.personalizations.length === 0) {
            ...
            const itemReviewReasons = itemAiResult.overallReviewReason
        ...
        for (const taskDetail of taskDetailsToCreate) {
            ...
            if (options.dryRun) {
                logger.info(`[Dry Run][DBTasks][Order ${order.id}][Item ${orderItemId}] Would upsert task ${taskDetail.taskIndex}. Review: ${taskDetail.needs_review}`);
                ...
                try {
                    ...
                    logger.debug(`[DBTasks][Order ${order.id}][Item ${orderItemId}][Task ${taskDetail.taskIndex}] Preparing to UPSERT task with data:`, upsertData // Be mindful of logging sensitive data
                    );
                    const task = await tx.printOrderTask.upsert(upsertData);
                    logger.info(`[DBTasks][Order ${order.id}][Item ${orderItemId}][Task ${taskDetail.taskIndex}] Upserted task ${task.id}.`);
                    tasksCreatedCount++;
                    itemDebugEntry.createdTaskIds.push(task.id);
                }
                catch (e) {
                    const errorMsg = e instanceof Error ? e.message : String(e);
                    logger.error(`[DBTasks][Order ${order.id}][Item ${orderItemId}] FAILED upsert task ${taskDetail.taskIndex}: ${errorMsg}`, e);
        ...
        logger.info(`[DBTasks][Order ${order.id}][Item ${orderItemId}] Finished DB processing. Status: ${itemDebugEntry.status}`);
    } // End item loop
...
exports.createOrUpdateTasksInTransaction = createOrUpdateTasksInTransaction;
