 ...
 */
export const upsertCustomerFromOrder = async (ssOrder, options // Added options parameter
) => {
 ...
 */
export const upsertProductFromItem = async (tx, ssItem, options // Added options parameter
) => {
 ...
 */
async function upsertOrderWithItems(orderData, // Type from ShipStation API response
progressId, options // Pass sync options
) {
        ...
        if (options?.dryRun) {
            ...
            const transactionResult = await prisma.$transaction(async (tx) => {
                ...
                if (!ssOrderIdValue) {
                    ...
                    logger.error(`[Sync][Order ${orderData.orderNumber}] Missing orderId from ShipStation data. Cannot process order.`);
                ...
                for (const ssItem of orderData.items) {
                    ...
                    try {
                        ...
                        if (!dbProduct) {
                            ...
                            logger.warn(`[Sync][Order ${orderData.orderNumber}][Item ${lineItemKey}] ${errorMsg}`);
                        ...
                        if (ssLineItemKey) {
                            ...
                            logger.warn(`[Sync][Order ${orderData.orderNumber}][Item SKU ${ssItem.sku || 'N/A'}] Missing shipstationLineItemKey. Creating new OrderItem. Potential duplicate if re-syncing.`);
                        ...
                        currentItemsFailed++;
                        const itemErrorMsg = itemError instanceof Error ? itemError.message : 'Unknown item error';
                        logger.error(`[Sync][Order ${orderData.orderNumber}][Item ${lineItemKey}] Error upserting item: ${itemErrorMsg}`, { error: itemError });
                ...
                logger.info(`[Sync][Order ${orderData.orderNumber}] Item Upsert Complete: ${currentItemsProcessed} items processed, ${currentItemsFailed} items failed.`);
 ...
 */
async function getShipstationOrders(params, retries = MAX_RETRIES) {
    recordMetric({
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            ...
            logger.warn(`[API Call] Failed to fetch orders (Status: ${status}). Retries left: ${retries}. Rate Limit Remaining: ${rateLimitRemaining ?? 'N/A'}`);
            if (status === 429 && retries > 0) {
                ...
                logger.warn(`[API Call] Server error (${status}). Waiting ${DELAY_MS / 1000}s before retrying...`);
                await new Promise(resolve => setTimeout(resolve, DELAY_MS));
                return getShipstationOrders(params, retries - 1);
            }
        }
        logger.error('[API Call] Error fetching ShipStation orders after retries or for non-retriable error:', { error: error instanceof Error ? error.message : error, params });
 ...
 */
export async function syncAllPaginatedOrders(options, overrideStartDate // Allow passing start date directly for recent sync
) {
    ...
    try {
            ...
            try {
                ...
                if (orders && orders.length > 0) {
                    ...
                    for (const orderData of orders) {
                        const result = await upsertOrderWithItems(orderData, progressId, // Pass progressId (string)
        ...
        if (overallSuccess) {
            logger.info(`[Full Sync] ShipStation full order sync completed successfully. Orders processed: ${ordersProcessed}, Orders failed: ${ordersFailed}`);
        }
        else {
            logger.warn(`[Full Sync] ShipStation full order sync completed with errors. Orders processed: ${ordersProcessed}, Orders failed: ${ordersFailed}`);
        }
        // Mark completion status after the loop finishes or breaks
        await markSyncCompleted(progressId, overallSuccess, overallSuccess ? undefined : 'Sync completed with errors');
        // Return final counts
        return {
 ...
 */
export async function syncRecentOrders(lookbackDays = 2, options) {
    const progressId = await createSyncProgress('recent'); // Create specific progress record
    let ordersProcessed = 0;
    let ordersFailed = 0;
    let overallSuccess = true;
    const allRecentOrders = []; // Array to hold all fetched orders
    try {
        const now = new Date();
        ...
        const lookbackPeriod = lookbackDays >= 1 ? `${lookbackDays} days` : `${Math.round(lookbackDays * 24)} hours`;
        logger.info(`[Recent Sync] Starting sync for orders modified in the last ${lookbackPeriod} (since ${dateStartFilter} UTC)${options?.dryRun ? ' (DRY RUN)' : ''}`);
        await updateSyncProgress(progressId, {
            status: 'running',
        ...
        for (const orderData of allRecentOrders) {
            const result = await upsertOrderWithItems(orderData, progressId, // Pass the specific progressId for recent sync
            options);
            if (result.success) {
                ordersProcessed++;
            }
        ...
        if (overallSuccess) {
            logger.info(`[Recent Sync] Completed successfully. Orders processed: ${ordersProcessed}, Orders failed: ${ordersFailed}${options?.dryRun ? ' (DRY RUN)' : ''}`);
        }
        else {
            logger.warn(`[Recent Sync] Completed with errors. Orders processed: ${ordersProcessed}, Orders failed: ${ordersFailed}${options?.dryRun ? ' (DRY RUN)' : ''}`);
        }
        await markSyncCompleted(progressId, overallSuccess, overallSuccess ? undefined : 'Recent sync completed with errors');
 ...
 */
export async function syncSingleOrder(orderIdentifier, options // Added options
) {
    const progressId = await createSyncProgress('single');
    let overallSuccess = false;
    let errorMsg;
    try {
        logger.info(`[Single Order Sync] Fetching order ${orderIdentifier} from ShipStation...${options?.dryRun ? ' (DRY RUN)' : ''}`);
        ...
        // Pass options down
        const result = await upsertOrderWithItems(orderData, progressId, // Pass progressId
        options);
        // Record metrics
        recordMetric({
            name: 'order_processed',
        ...
        if (result.success) {
            logger.info(`[Single Order Sync] Successfully synced order ${orderIdentifier}${options?.dryRun ? ' (DRY RUN)' : ''}`);
