...
import { Prisma, PrintTaskStatus } from '@prisma/client';
// Use relative import instead of path alias to fix module resolution
import { prisma } from '../prisma';
import logger from '../logger';
import { listTags } from './api';
import { mapAddressToCustomerFields, mapSsItemToProductData, mapSsItemToOrderItemData, mapOrderToPrisma, } from './mappers';
 ...
 */
export const upsertCustomerFromOrder = async (ssOrder, options // Add options parameter
) => {
    ...
    if (!shipTo) {
        logger.warn(`[Sync][Order ${ssOrder.orderNumber}] No shipTo address found. Cannot effectively upsert customer address details.`);
    }
    if (!email) {
        logger.warn(`[Sync][Order ${ssOrder.orderNumber}] Customer email is missing. Cannot reliably upsert customer based on email.`);
        return null;
    }
    try {
        ...
        let customer = null; // Initialize as null
        ...
        if (existingCustomer) {
            ...
            if (options?.dryRun) {
                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would update customer ${existingCustomer.name} (ID: ${existingCustomer.id}) with data:`, customerUpdateData);
                ...
                customer = {
                    ...
                    company: customerUpdateData.company,
                    street1: customerUpdateData.street1,
                    street2: customerUpdateData.street2,
                    street3: customerUpdateData.street3,
                    city: customerUpdateData.city,
                    state: customerUpdateData.state,
                    postal_code: customerUpdateData.postal_code,
                    country_code: customerUpdateData.country_code,
            ...
            if (options?.dryRun) {
                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would create customer with data:`, customerCreateData);
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
            logger.error(`[Sync][Order ${ssOrder.orderNumber}] Failed to upsert customer due to unique constraint. Email: ${email}, SS_ID: ${shipstationCustomerIdStr}. Fields: ${error.meta?.target}`, { error });
        }
        else {
            logger.error(`[Sync][Order ${ssOrder.orderNumber}] Error upserting customer. Email: ${email}, SS_ID: ${shipstationCustomerIdStr}:`, { error });
        }
        return null;
    }
};
 ...
 * Includes dry run logic.
 */
export const upsertProductFromItem = async (tx, // Expecting Prisma Transaction Client
ssItem, options // Add options parameter
) => {
    const trimmedSku = ssItem.sku?.trim();
    const shipstationProductId = ssItem.productId;
    if (!trimmedSku && !shipstationProductId) {
        logger.warn(`Product upsert skipped: Item missing SKU and SS Product ID. Name: ${ssItem.name || '(No Name)'}, LineItemKey: ${ssItem.lineItemKey}`);
        return null;
    }
    ...
    try {
        if (trimmedSku) {
            ...
            if (existingBySku) {
                logger.info(`[Product Sync] Found existing product by SKU: ${trimmedSku} (ID: ${existingBySku.id}). Updating...`);
                // ... existing conflict warning logic ...
                if (shipstationProductId !== undefined &&
                    existingBySku.shipstation_product_id !== undefined &&
                    existingBySku.shipstation_product_id !== shipstationProductId) {
                    logger.warn(`[Product Sync Conflict] SKU '${trimmedSku}' exists (DB ID: ${existingBySku.id}) but with different ShipStation Product ID. ` +
                ...
                if (options?.dryRun) {
                    logger.info(`[Product Sync][Dry Run] Would update product by SKU ${trimmedSku} (ID: ${existingBySku.id}) with data:`, updateData);
                    ...
                    try {
                        ...
                        if (updateError instanceof Prisma.PrismaClientKnownRequestError &&
                            ...
                            logger.warn(`[Product Sync Conflict] Update failed for SKU '${trimmedSku}' (ID: ${existingBySku.id}). ` +
                            ...
                            return existingBySku; // Return the original existing product
                        }
                        else {
                            logger.error(`[Product Sync] Unexpected error updating product by SKU '${trimmedSku}' (ID: ${existingBySku.id}):`, { error: updateError });
                            throw updateError;
                        }
                    }
                }
        ...
        // Upsert/Create logic
        const createInput = productData;
        const updateInput = productData;
        if (shipstationProductId) {
            logger.info(`[Product Sync] Upserting by SS Product ID: ${shipstationProductId} (SKU: ${trimmedSku || 'N/A'})...`);
            if (options?.dryRun) {
                logger.info(`[Product Sync][Dry Run] Would upsert product by SS_ID ${shipstationProductId}. Create data:`, createInput, 'Update data:', updateInput);
                ...
                logger.info(`[Product Sync] Upserted by SS Product ID: ${product.name} (ID: ${product.id})`);
                return product;
            }
        }
        else if (trimmedSku) {
            logger.warn(`[Product Sync] Attempting CREATE by SKU only: ${trimmedSku} (SS Product ID missing)...`);
            if (options?.dryRun) {
                logger.info(`[Product Sync][Dry Run] Would create product by SKU only ${trimmedSku}. Data:`, createInput);
                // Use a negative number for mock ID
                ...
                logger.info(`[Product Sync] Created product by SKU only: ${product.name} (ID: ${product.id})`);
                return product;
            }
        }
        else {
            logger.error('[Product Sync] Logical error: No identifier (SKU or SS Product ID) for upsert.');
            return null;
        }
    }
    catch (error) {
        logger.error(`[Product Sync] Error during product sync for SKU: ${trimmedSku}, SS_ID: ${shipstationProductId}. Error:`, { error });
 ...
 * Includes dry run logic.
 */
export const upsertOrderWithItems = async (ssOrder, options // Add options parameter
) => {
    const shipstationOrderIdStr = ssOrder.orderId.toString();
    logger.info(`[Sync] Starting upsert process for Order SS_ID: ${shipstationOrderIdStr} (Number: ${ssOrder.orderNumber})`);
    if (options?.dryRun) {
        logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Running in dry run mode.`);
    }
    const errors = [];
    let previousOrderStatus; // To track status changes
    try {
        ...
        if (!customer) {
            ...
            logger.error(`[Sync][Order ${ssOrder.orderNumber}] Failed to upsert customer, continuing order sync...`);
        ...
        if (options?.dryRun) {
            logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] --- Start Transaction Simulation ---`);
            // Simulate Order Upsert
            const mockOrderId = existingOrder?.id ?? `dry-run-order-${shipstationOrderIdStr}`;
            if (existingOrder) {
                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would update order (ID: ${mockOrderId}) with data:`, orderInputData);
            }
            else {
                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would create order with data:`, orderInputData);
            }
            // Simulate Item Processing
            const incomingSsItems = ssOrder.items.filter(item => !item.adjustment);
            logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Simulating processing ${incomingSsItems.length} items...`);
            ...
            for (const ssItem of incomingSsItems) {
                ...
                try {
                    ...
                    const dbProduct = await upsertProductFromItem(prisma, ssItem, options);
                    if (!dbProduct) {
                        const errorMsg = `Product could not be upserted. SKU: ${ssItem.sku}, Name: ${ssItem.name}`;
                        logger.warn(`[Sync][Dry Run][Order ${ssOrder.orderNumber}][Item ${lineItemKey}] ${errorMsg}`);
                    ...
                    if (existingItem) {
                        logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}][Item ${lineItemKey}] Would update OrderItem (ID: ${existingItem.id}) with data:`, updateData);
                    }
                    else {
                        logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}][Item ${lineItemKey}] Would create OrderItem with data:`, createData);
                    }
                    itemsProcessed++;
                }
                catch (itemError) {
                    const errorMsg = itemError instanceof Error ? itemError.message : String(itemError);
                    logger.error(`[Sync][Dry Run][Order ${ssOrder.orderNumber}][Item ${lineItemKey}] Failed to simulate item processing: ${errorMsg}`, { error: itemError });
                    errors.push({ itemId: lineItemKey, error: errorMsg });
                    itemsFailed++;
                }
            } // End item loop simulation
            logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Item Upsert Simulation Complete: ${itemsProcessed} processed, ${itemsFailed} failed.`);
            ...
            if (ssOrder.orderStatus === 'shipped' || ssOrder.orderStatus === 'cancelled') {
                ...
                if (previousOrderStatus && previousOrderStatus !== ssOrder.orderStatus) {
                    ...
                    if (shouldAutoComplete) {
                        logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would check for pending/in-progress tasks for order ID ${mockOrderId} to auto-complete.`);
                        ...
                        if (typeof orderIdToCheck === 'number') {
                            ...
                            if (pendingTasks.length > 0) {
                                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Would auto-complete ${pendingTasks.length} print tasks.`);
                            }
                            else {
                                logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] No pending/in-progress print tasks found to auto-complete.`);
                            }
                        }
                        else {
                            logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] Skipping print task check because Order ID is a mock value (${orderIdToCheck}).`);
                        }
                    }
                }
            }
            logger.info(`[Sync][Dry Run][Order ${ssOrder.orderNumber}] --- End Transaction Simulation ---`);
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const dbOrder = await tx.order.upsert({
                where: { shipstation_order_id: shipstationOrderIdStr },
                update: orderInputData,
                create: orderInputData,
                select: { id: true },
            });
            const dbOrderId = dbOrder.id;
            // Process Incoming Items using Upsert
            const incomingSsItems = ssOrder.items.filter(item => !item.adjustment);
            logger.info(`[Sync][Order ${ssOrder.orderNumber}] Processing ${incomingSsItems.length} incoming items using upsert...`);
            let itemsProcessed = 0;
            let itemsFailed = 0;
            for (const ssItem of incomingSsItems) {
                if (!ssItem.lineItemKey) {
                ...
                try {
                    ...
                    const errorMsg = itemError instanceof Error ? itemError.message : String(itemError);
                    logger.error(`[Sync][Order ${ssOrder.orderNumber}][Item ${lineItemKey}] Failed to process item: ${errorMsg}`, { error: itemError });
                    errors.push({ itemId: lineItemKey, error: errorMsg });
                    itemsFailed++;
                    // Continue with next item instead of failing the entire transaction
                }
            } // End item loop
            logger.info(`[Sync][Order ${ssOrder.orderNumber}] Item Upsert Complete: ${itemsProcessed} processed, ${itemsFailed} failed.`);
            ...
            if (ssOrder.orderStatus === 'shipped' || ssOrder.orderStatus === 'cancelled') {
                ...
                if (previousOrderStatus && previousOrderStatus !== ssOrder.orderStatus) {
                    ...
                    const shouldAutoComplete = validPreviousStatuses.includes(previousOrderStatus);
                    if (shouldAutoComplete) {
                        logger.info(`[Sync][Order ${ssOrder.orderNumber}] Order status changed from ${previousOrderStatus} to ${ssOrder.orderStatus}. Auto-completing print tasks...`);
                        ...
                        if (pendingTasks.length > 0) {
                            ...
                            logger.info(`[Sync][Order ${ssOrder.orderNumber}] Auto-completed ${pendingTasks.length} print tasks.`);
                        }
                        else {
                            logger.info(`[Sync][Order ${ssOrder.orderNumber}] No pending or in-progress print tasks found to auto-complete.`);
                        }
                    }
                    else {
                        logger.info(`[Sync][Order ${ssOrder.orderNumber}] Order status changed from ${previousOrderStatus} to ${ssOrder.orderStatus}, but previous status is not one that triggers auto-completion.`);
                    }
                }
                else {
                    logger.info(`[Sync][Order ${ssOrder.orderNumber}] Order status is ${ssOrder.orderStatus} but no status change detected.`);
                }
            }
        ...
        logger.info(`[Sync] Successfully processed Order ${result.order.shipstation_order_number}. Items: ${result.itemsProcessed} processed, ${result.itemsFailed} failed.`);
        return {
            order: result.order,
            success: true,
 ...
 * Includes dry run logic.
 */
export async function syncShipStationTags(options) {
    ...
    try {
        ...
        for (const ssTag of ssTags) {
            ...
            if (options?.dryRun) {
                ...
                if (existingTag) {
                    logger.info(`[Sync Tags][Dry Run] Would update tag ${ssTag.name} (SS_ID: ${ssTag.tagId}) with data:`, updateData);
                }
                else {
                    logger.info(`[Sync Tags][Dry Run] Would create tag ${ssTag.name} (SS_ID: ${ssTag.tagId}) with data:`, createData);
                }
                processedCount++;
            }
            else {
        ...
        logger.info(`[Sync Tags] Finished. ${options?.dryRun ? 'Would have processed' : 'Processed'} ${processedCount} tags from ShipStation.`);
    }
    catch (error) {
