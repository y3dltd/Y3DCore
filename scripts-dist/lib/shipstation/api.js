 ...
 */
export const getShipstationOrders = async (params = {}) => {
    let attempt = 0;
    ...
    while (attempt < MAX_RETRIES) {
        ...
        try {
            ...
            if (orderIdToFetch) {
                ...
                console.log(`[API] Attempt ${attempt}: Fetching single order from ShipStation: ${endpoint}`);
                // Fetch single order - response data is the ShipStationOrder object directly
                response = await shipstationApi.get(endpoint);
                // Wrap the single order in the expected list response structure
                const singleOrderData = response.data;
                console.log(`[API] Fetched single order ${singleOrderData.orderNumber} (ID: ${singleOrderData.orderId}). Status: ${singleOrderData.orderStatus}`);
                return {
                ...
                console.log(`[API] Attempt ${attempt}: Fetching order list from ShipStation with params:`, params);
                response = await shipstationApi.get('/orders', { params } // Pass the original params for filtering/pagination
                );
                console.log(`[API] Fetched ${response.data.orders.length} orders from page ${response.data.page}/${response.data.pages}. Total: ${response.data.total}`);
 ...
 */
export const syncShipstationData = async (syncParams = {}) => {
    let currentPage = 1;
    let totalPages = 1;
    ...
    try {
        do {
            ...
            if (!response || !response.orders) {
                throw new Error(`[Sync] Inconsistent state: Failed to get orders response for page ${currentPage}, but no error thrown.`);
            }
            if (currentPage === 1) {
            ...
            console.log(`[Sync] Processing ${response.orders.length} orders from page ${currentPage}/${totalPages === Infinity ? '?' : totalPages}...`);
...
export async function updateOrderItemOptions(lineItemKey, newOptions, fetchedOrder // Accept the full fetched order object
) {
...
// ... existing code ...
export async function updateOrderItemsOptionsBatch(fetchedOrder, itemsToPatch, auditNote = null) {
    const endpoint = '/orders/createorder';
    ...
    const mergedNotes = [...existingLines, ...newNotesLines].filter((line, idx, arr) => line && arr.indexOf(line) === idx);
    const payload = {
        ...fetchedOrder,
        items: updatedItems,
    ...
    if (payload.orderId)
        payload.orderId = Number(payload.orderId);
    console.log(`[ShipStation API] Batch‑updating ${Object.keys(itemsToPatch).length} items in order ${fetchedOrder.orderId} …`);
    try {
        console.log('[ShipStation API] Sending payload:', JSON.stringify(payload, null, 2));
        const response = await shipstationApi.post(endpoint, payload);
        if (response.status === 200 || response.status === 201) {
            console.log(`[ShipStation API] Batch update success for order ${fetchedOrder.orderId}.`);
            return true;
        }
        console.warn(`[ShipStation API] Batch update unexpected status ${response.status} for order ${fetchedOrder.orderId}.`);
        return false;
    }
    catch (error) {
