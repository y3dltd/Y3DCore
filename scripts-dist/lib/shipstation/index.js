...
import { createSyncProgress, updateSyncProgress, markSyncCompleted, incrementProcessedOrders, incrementFailedOrders, updateLastProcessedOrder, } from './sync-progress';
export * from './types';
export * from './api';
...
defaultStartDate = '2022-01-01T00:00:00.000Z', 
...
options) {
    ...
    try {
        ...
        while (page <= totalPages && success) {
            ...
            try {
                ...
                if (page === 1) {
                    ...
                    logger.info(`[Full Sync] Total pages reported: ${totalPages === Infinity ? 'Unknown' : totalPages}`);
                    if (total) {
                ...
                if (orders && orders.length > 0) {
                    ...
                    for (const orderData of orders) {
                        ...
                        try {
                            ...
                            metrics.recordOrderProcessed(orderData.orderId.toString(), result.success, result.itemsProcessed, result.itemsFailed);
                            if (result.success) {
                                ordersProcessed++;
                                ...
                                if (!modifyDateValue) {
                                    logger.warn(`[Full Sync] Order ${orderData.orderNumber} (SS_ID: ${orderData.orderId}) has undefined modifyDate. Using current time for lastProcessedOrder update.`);
                                    // Use current time directly for updateLastProcessedOrder
                                    await updateLastProcessedOrder(progressId, orderData.orderId.toString(), new Date() // Use current time
                                    );
                                }
                                else {
                                    // Only create Date if modifyDateValue is valid
                                    const modifyDate = new Date(modifyDateValue);
                                    await updateLastProcessedOrder(progressId, orderData.orderId.toString(), modifyDate);
                                }
                            }
                            else {
                                ordersFailed++;
                            ...
                            logger.error(`[Full Sync] Error processing order ${orderData.orderNumber}: ${errorMsg}`, { error: orderError });
        ...
        if (success) {
            logger.info(`[Full Sync] ShipStation full order sync completed successfully. Processed: ${ordersProcessed}, Failed: ${ordersFailed}`);
        }
        else {
            logger.error(`[Full Sync] ShipStation full order sync failed. Processed: ${ordersProcessed}, Failed: ${ordersFailed}`);
        }
        // Mark progress as completed or failed
        await markSyncCompleted(progressId, success, success ? undefined : 'Sync failed during page processing.');
        // Save metrics
 ...
 */
export async function syncRecentOrders(lookbackDays = 2, 
// Add SyncOptions parameter
options) {
    ...
    try {
        ...
        logger.info(`[Recent Sync] Starting sync for orders in the last ${lookbackPeriod} (since ${dateStartFilter})`);
        ...
        await markSyncCompleted(progressId, result.success, result.success ? undefined : 'Recent sync failed.');
        ...
        logger.info(`[Recent Sync] Completed. Success: ${result.success}, Processed: ${result.ordersProcessed}, Failed: ${result.ordersFailed}`);
        return result;
    }
    catch (error) {
 ...
 */
export async function syncSingleOrder(orderId, // Assuming orderId is the ShipStation Order ID (string)
// Add SyncOptions parameter
options) {
    ...
    try {
        ...
        metrics.recordOrderProcessed(orderId, result.success, result.itemsProcessed, result.itemsFailed);
        if (result.success) {
            logger.info(`[Single Order Sync] Successfully synced order ${orderId}`);
            await incrementProcessedOrders(progressId);
            // Use string orderId for updating last processed order
            await updateLastProcessedOrder(progressId, orderId, new Date() // Use current time for single sync
            );
