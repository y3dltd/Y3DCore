...
export const mapSsItemToProductData = (ssItem) => ({
...
export const mapSsItemToOrderItemData = (ssItem, productId) => ({
    ...
    print_settings: ssItem.options && ssItem.options.length > 0
 ...
 */
function convertShipStationDateToUTC(dateString) {
    if (!dateString)
        return null;
...
export const mapOrderToPrisma = (ssOrder, dbCustomerId) => ({
    ...
    customer_name: ssOrder.shipTo?.name || ssOrder.billTo?.name || ssOrder.customerUsername || 'Unknown Customer',
    ...
    tag_ids: ssOrder.tagIds && ssOrder.tagIds.length > 0
