...
const sp_api_1 = require("../lib/amazon/sp-api");
...
async function findCustomizationUrl(orderId) {
    console.log(`--- Fetching customization for Order ID: ${orderId} ---`);
    try {
        const orderItemsResponse = await (0, sp_api_1.getOrderItems)(orderId);
        ...
        console.error(`--- Failed to fetch or process customization for Order ID: ${orderId} ---`, error);
