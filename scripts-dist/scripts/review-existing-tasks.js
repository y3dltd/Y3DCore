...
const _AiOrderResponseSchema = zod_1.default.object({
...
async function extractOrderPersonalization(order, options) {
...
// --- Main Update Logic ---
async function applyAiUpdatesToTasks(tx, orderId, itemId, _itemQuantity, // Pass item quantity for context (unused)
existingTasks, aiPersonalizations, options // Pass simplified options
) {
    const warnings = [];
    let updatedCount = 0;
    let createdCount = 0;
    ...
    if (aiCount === dbCount) {
        ...
        for (let i = 0; i < aiCount; i++) {
            ...
            if (dbTask.custom_text !== aiTask.customText) {
                updates.custom_text = aiTask.customText;
                needsUpdate = true;
            }
            if (dbTask.color_1 !== aiTask.color1) {
                updates.color_1 = aiTask.color1;
                needsUpdate = true;
            }
            if (dbTask.color_2 !== aiTask.color2) {
                updates.color_2 = aiTask.color2;
                needsUpdate = true;
            }
            if (dbTask.quantity !== aiTask.quantity) {
                updates.quantity = aiTask.quantity;
                needsUpdate = true;
            }
            if (dbTask.needs_review !== aiTask.needsReview) {
                updates.needs_review = aiTask.needsReview;
                needsUpdate = true;
            }
            if (dbTask.review_reason !== aiTask.reviewReason) {
                updates.review_reason = aiTask.reviewReason;
                needsUpdate = true;
            }
            if (dbTask.annotation !== aiTask.annotation) {
                updates.annotation = aiTask.annotation;
                needsUpdate = true;
            } // Also update annotation
            // Status update logic
            if (aiTask.needsReview && dbTask.status !== client_1.PrintTaskStatus.completed) {
        ...
        for (let i = 0; i < dbCount; i++) {
            ...
            if (needsUpdate) {
                if (options.dryRun) {
                    ...
                    await tx.printOrderTask.update({ where: { id: dbTask.id }, data: updates });
                }
                updatedCount++;
            }
        }
...
async function main() {
    ...
    let cmdOptions; // Define cmdOptions here
    try {
        // Setup Logger
        ...
        logger = (0, pino_1.default)({ level: LOG_LEVEL }, pino_1.default.multistream([{ stream: logStream }, { stream: process.stdout }])); // Use LOG_LEVEL constant
