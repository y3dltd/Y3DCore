...
const client_1 = require("@prisma/client"); // <-- Fix: Add PrintOrderTask and PrintTaskStatus
...
async function loadPromptFile(filePath) {
    try {
        ...
        if (logger)
            ...
            console.error(`Failed to load prompt file: ${filePath} - ${errorMsg}`);
...
async function extractOrderPersonalization(order, // Use the correct type
latestCustomerNotes, // Add parameter for potentially updated notes
options) {
...
// --- Main Update Logic ---
async function applyAiUpdatesToTasks(tx, orderId, itemId, _itemQuantity, // Unused but kept for API compatibility
existingTasks, aiPersonalizations, options, notesColor1, // Explicitly parsed color 1 from notes
notesColor2 // Explicitly parsed color 2 from notes
) {
    const warnings = [];
    let updatedCount = 0;
    let createdCount = 0;
    ...
    const finalAiTasksForShipStation = [];
    ...
    const getCorrectedAiTaskForShipStation = (originalAiTask) => {
    ...
    if (aiCount === dbCount) {
        ...
        if (!orderItem || !orderItem.order || !orderItem.product) {
            ...
            for (let i = dbCount; i < aiCount; i++) {
                ...
                const newTaskData = {
                    ...
                    color_1: notesColor1 ?? originalAiTask.color1,
                    color_2: notesColor2 ?? originalAiTask.color2,
        ...
        for (let i = aiCount; i < dbCount; i++) {
            ...
            if (dbTaskToDelete.status === client_1.PrintTaskStatus.pending) {
                if (options.dryRun) {
...
async function main() {
    ...
    let cmdOptions;
    try {
        logger = (0, pino_1.default)({ level: LOG_LEVEL }, process.stdout);
        logger.info(`--- Script Start: ${new Date().toISOString()} ---`);
        const openaiApiKey = process.env.OPENAI_API_KEY ?? null;
        if (!openaiApiKey)
            throw new Error('OpenAI API key missing (check .env).');
...
`;
        ...
        for (const order of ordersToUpdate) {
            ...
            if (order.shipstation_order_id) {
                ...
                try {
                    const ssOrderResp = await (0, shipstation_1.getShipstationOrders)({ orderId: Number(order.shipstation_order_id) });
            ...
            const orderItemsToPatch = {};
            const orderPatchReasons = [];
            let dbUpdatesMadeThisOrder = false;
            // --- Explicit notes parsing for DB comparison --- START
            let notesColor1 = null;
            let notesColor2 = null;
            ...
            try {
                await prisma.$transaction(async (tx) => {
                    for (const item of order.items) {
                        ...
                        const result = await applyAiUpdatesToTasks(tx, order.id, item.id, item.quantity, existingTasks, aiPersonalizationsForItem, cmdOptions, notesColor1, notesColor2);
                        ...
                        if (!cmdOptions.dryRun && item.shipstationLineItemKey && result.aiTasksForShipStation.length > 0) {
                            ...
                            const ssOptions = [];
                            if (primaryAiTask.customText)
                                ssOptions.push({ name: 'Name or Text', value: primaryAiTask.customText });
                            if (primaryAiTask.color1)
                                ssOptions.push({ name: 'Colour 1', value: primaryAiTask.color1 });
                            if (primaryAiTask.color2)
                                ssOptions.push({ name: 'Colour 2', value: primaryAiTask.color2 });
                ...
                if (!cmdOptions.dryRun && Object.keys(orderItemsToPatch).length > 0 && order.shipstation_order_id && (dbUpdatesMadeThisOrder || FORCE_SHIPSTATION_UPDATE)) {
                    ...
                    try {
                        ...
                        if (ssOrderResp.orders && ssOrderResp.orders.length > 0) {
                            ...
                            await (0, shipstation_1.updateOrderItemsOptionsBatch)(ssOrderResp.orders[0], orderItemsToPatch, auditNote);
        ...
        if (logger)
            logger.info('DB disconnected. Script finished.');
        else
            console.log('DB disconnected. Script finished.');
    }
}
void main();
