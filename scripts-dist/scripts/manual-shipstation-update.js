...
const shipstation_1 = require("../lib/shared/shipstation"); // Assuming these are correctly exported
...
async function runManualUpdate(options) {
    ...
    try {
        ...
        for (const task of tasks) {
            if (task.orderItem?.shipstationLineItemKey &&
                ...
                logger.warn(`Task ID ${task.id} for Order ID ${options.orderId} is missing shipstationLineItemKey. Skipping.`);
            }
            else if (!task.order?.shipstation_order_id) {
                logger.warn(`Task ID ${task.id} for Order ID ${options.orderId} is missing shipstation_order_id on the parent order. Skipping.`);
            }
        }
        if (itemsToUpdate.size === 0) {
            logger.error(`No tasks with required ShipStation keys found for Order ID ${options.orderId}.`);
            return;
        }
        ...
        const ssOrderResponse = await (0, shipstation_1.getShipstationOrders)({ orderId: Number(shipstationOrderId) });
        if (!ssOrderResponse || !ssOrderResponse.orders || ssOrderResponse.orders.length === 0) {
            logger.error(`Failed to fetch order details from ShipStation for SS Order ID: ${shipstationOrderId}.`);
            return;
        }
        ...
        for (const [lineItemKey, data] of itemsToUpdate.entries()) {
            const { task } = data;
            logger.info(`Processing update for LineItemKey: ${lineItemKey} using data from Task ID: ${task.id}`);
            ...
            if (ssOptions.length === 0) {
                logger.warn(`No options constructed from Task ID ${task.id} for LineItemKey ${lineItemKey}. Skipping update.`);
                continue;
            }
            logger.debug(`Constructed options for ${lineItemKey}:`, ssOptions);
            // 5. Call update function
            if (options.dryRun) {
                logger.info(`[Dry Run] Would call updateOrderItemOptions for LineItemKey ${lineItemKey} with options: ${JSON.stringify(ssOptions)}`);
            }
            else {
                logger.info(`Calling updateOrderItemOptions for LineItemKey ${lineItemKey}...`);
                try {
                    const updateSuccess = await (0, shipstation_1.updateOrderItemOptions)(lineItemKey, ssOptions, ssOrder // Pass the fetched ShipStation order object
                    );
                    if (updateSuccess) {
                        logger.info(`Successfully updated options for LineItemKey ${lineItemKey}.`);
                    }
                    else {
                        logger.error(`Failed to update options for LineItemKey ${lineItemKey} (updateOrderItemOptions returned false).`);
                        // Consider adding to a list of failed items
                    }
                }
                catch (updateError) {
                    logger.error(`Error calling updateOrderItemOptions for LineItemKey ${lineItemKey}:`, updateError);
                    // Consider adding to a list of failed items
                }
            }
        }
        ...
        logger.error(`An unexpected error occurred during manual update for Order ID ${options.orderId}:`, error);
    }
    finally {
...
async function main() {
    ...
    program
        ...
        .description('Manually push personalization details from DB PrintOrderTasks to ShipStation item options.')
