...
export async function PATCH(
  ...
  try {
    ...
    if (typeof error === 'object' && error !== null && 'code' in error && error.code === 'P2025') {
        return NextResponse.json({ error: `User with ID ${params.userId} not found.` }, { status: 404 });
    }
  ...
  { params }: { params: { userId: string } }
    ...
    try {
        ...
        if (userId === ADMIN_USER_ID) {
             return NextResponse.json({ error: 'Cannot delete the primary admin user (ID 1).' }, { status: 403 }); // Forbidden
        }
