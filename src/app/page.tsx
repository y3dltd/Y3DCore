    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-700 text-primary-foreground p-6 rounded-lg shadow-lg">
        ...
        <p className="text-sm opacity-90 max-w-3xl">
          Use search and filtering options to easily locate orders by number or customer name, also view your metrics by
          other criteria or marketplaces, view metrics and manage metrics and manage tasks.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
        <div className="md:col-span-1 bg-card text-card-foreground rounded-lg border p-4 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground mb-3 text-center">Pull latest orders from ShipStation.</p>
          <SyncButton />
        </div>
      ...
      {/* Widgets Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4"> {/* Grid for widgets */}
                ...
                {recentOrders.length > 0 ? (
                  recentOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="truncate max-w-[150px]">{order.customer_name || 'N/A'}</TableCell>
                      <TableCell>{order.marketplace || 'N/A'}</TableCell>
                      <TableCell className="text-right">{CURRENCY_SYMBOL}{Number(order.total_price).toFixed(2)}</TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground">No recent orders found.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Link href="/orders?status=on_hold" className="block p-3 rounded-md hover:bg-muted/50 border border-transparent hover:border-border transition-colors">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Orders On Hold</span>
                <Badge variant={ordersOnHoldCount > 0 ? "destructive" : "secondary"}>{ordersOnHoldCount}</Badge>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Review orders marked as &apos;on_hold&apos;.</p>
            </Link>
            <Link href="/print-queue?review=true" className="block p-3 rounded-md hover:bg-muted/50 border border-transparent hover:border-border transition-colors">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Tasks Needing Review</span>
                <Badge variant={stats.tasksNeedReview > 0 ? "destructive" : "secondary"}>{stats.tasksNeedReview}</Badge>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Review print tasks flagged by the system.</p>
            </Link>
          </CardContent>
        </Card>
