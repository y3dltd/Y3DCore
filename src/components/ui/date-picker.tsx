...
export function DatePicker({ name, value, placeholder = "Pick a date", onSelect }: DatePickerProps) {
    ...
    React.useEffect(() => {
        const newDate = value && /\d{4}-\d{2}-\d{2}/.test(value)
    ...
    return (
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}> {/* Controlled popover */}
            ...
            <PopoverContent className="w-auto p-0 space-y-2">
                ...
                <div className="flex justify-around p-2 border-b">
                    <Button variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={setToday}>Today</Button>
                    <Button variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={setTomorrow}>Tomorrow</Button>
                    <Button variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={setNextWeek}>Next Week</Button>
                    <Button variant="ghost" size="sm" className="h-7 text-xs px-2 text-red-500 hover:text-red-600" onClick={clearDate}>Clear</Button>
                </div>
                <Calendar
                    mode="single"
                    selected={date}
