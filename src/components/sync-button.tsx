...
export function SyncButton() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      } catch (_ignoredError) { // Rename and add eslint disable comment for unused var
         // Handle cases where the response is not JSON (e.g., server error HTML page)
         throw new Error(`Failed to parse response. Status: ${response.status} ${response.statusText}`);
      }
  ...
  return (
    <div className="mt-4 p-4 border rounded-lg bg-card text-card-foreground">
      <Button onClick={handleSync} disabled={isLoading || !SYNC_API_KEY_FRONTEND} className="flex items-center gap-2">
      ...
      <div className="mt-3 text-sm">
        {summary && (
          <div className={`p-3 rounded-md ${summary.success ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300'}`}>
            ...
            {summary.success && (
              <ul className="list-disc list-inside mt-1 pl-2">
                ...
                <li>Pages Synced: {summary.pagesSynced} / {summary.totalPagesAvailable}</li>
              </ul>
            )}
          </div>
        )}
