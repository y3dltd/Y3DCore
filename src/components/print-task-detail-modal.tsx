...
export function PrintTaskDetailModal({ task, isOpen, onOpenChange }: PrintTaskDetailModalProps) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        setFormData(prev => prev ? { ...prev, [name]: type === 'number' ? parseInt(value, 10) || 0 : inputValue } : null);
    };
    ...
    // Handler for Checkbox changes (needs_review)
     const handleNeedsReviewChange = (checked: boolean | 'indeterminate') => {
         if (typeof checked === 'boolean') { // Handle indeterminate case if needed
    ...
    const handleSave = () => {
        ...
        startSaveTransition(async () => {
            try {
                const response = await fetch(`/api/print-tasks/${task.id}`, { // Use a PUT/PATCH route for full update
                    method: 'PATCH', // Or PUT
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            ...
            <DialogContent className="sm:max-w-[800px]">
                <DialogHeader className="border-b pb-3 mb-4">
                    <DialogTitle className="text-xl font-semibold">Edit Task Details (ID: {task.id})</DialogTitle>
                    <DialogDescription>
                        Order #{task.marketplace_order_number}
                        {task.ship_by_date && ` - Ship By: ${format(new Date(task.ship_by_date), 'dd/MM/yyyy')}`}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 max-h-[60vh] overflow-y-auto pr-2">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <Input id="product_name" name="product_name" value={formData.product_name} onChange={handleChange} className="flex-grow" />
                                <Button 
                                    variant="outline"
                                    size="sm"
                                    onClick={handleBulkNameUpdate}
                                    disabled={isBulkNameUpdating || !formData.product_name || formData.product_name === task.product?.name}
                                    title="Update name for all tasks with same original SKU or Name"
                                >
                                    {isBulkNameUpdating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} 
                        ...
                        <div>
                            ...
                            <Input id="quantity" name="quantity" type="number" value={formData.quantity} onChange={handleChange} min="0" />
                        </div>
                        <div>
                    ...
                    <div className="space-y-4">
                       <div>
                            ...
                            <Textarea id="custom_text" name="custom_text" value={formData.custom_text} onChange={handleChange} rows={5} />
                        </div>
                       <div className="flex items-center space-x-2 pt-2">
                            <Checkbox id="needs_review" name="needs_review" checked={formData.needs_review} onCheckedChange={handleNeedsReviewChange} />
                        ...
                        <div>
                            ...
                            <Textarea id="review_reason" name="review_reason" value={formData.review_reason} onChange={handleChange} rows={3} disabled={!formData.needs_review} />
                        </div>
                    </div>
                </div>
