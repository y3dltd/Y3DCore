...
// Import specific sync functions and SyncOptions
import {
...
const argvBuilder = yargs(hideBin(process.argv))
  .option("mode", {
    ...
    description:
      "Sync mode: all orders, recent orders, single order, or just tags",
  ...
  .option("force-start-date", {
    ...
    type: "string",
    description:
      "Force sync to start from this date (YYYY-MM-DD or ISO format)",
  })
...
async function main(args: ArgvType) {
  ...
  if (syncOptions.dryRun) {
    logger.warn(
      "[Script][Dry Run] DRY RUN MODE ENABLED. No database changes will be made."
    );
  }
  ...
  try {
    ...
    if (!args.skipTags && args.mode !== "single" && args.mode !== "tags") {
      ...
      logger.info(
        `[Script] Tag-only sync finished in ${(endTime - startTime) / 1000} seconds.`
      );
      process.exit(0);
    } else {
      logger.info(
        "[Script] Skipping tag sync (--skip-tags specified or single order mode)."
      );
    }
    ...
    switch (
      ...
      case "recent":
        ...
        if (args.hours !== undefined) {
          logger.info(
            `[Script] Starting recent orders sync (${args.hours} hours)...`
          );
          ...
          logger.info(
            `[Script] Starting recent orders sync (${args.lookbackDays} days)...`
          );
          // Pass options to syncRecentOrders
          syncResult = await syncRecentOrders(
            Number(args.lookbackDays),
            syncOptions
          );
        }
        break;
    ...
    if (syncResult.success) {
      ...
      logger.error(
        `[Script] Sync failed. Error: ${syncResult.error || "Unknown error"}`
      );
    }
  } catch (error) {
    ...
    logger.error(
      `[Script] Sync process failed (Mode: ${args.mode}): ${errorMsg}`,
      { error }
    );
    const endTime = Date.now();
    logger.info(
