...
async function runManualUpdate(options: ScriptOptions): Promise<void> {
  ...
  try {
    ...
    const itemsToUpdate = new Map<string, { task: PrintOrderTask, orderItem: OrderItem, order: Order }>();

    for (const task of tasks) {
        if (task.orderItem?.shipstationLineItemKey && task.order?.shipstation_order_id && !itemsToUpdate.has(task.orderItem.shipstationLineItemKey)) {
