    ...
    try {
        ...
        program
            ...
            .option(
                ...
                'Only sync existing DB tasks to ShipStation (Not fully implemented in V2 orchestrator yet)', // Mark as potentially not ready
                false
            )
        ...
        const logger = await initializeLogger(loggerOptions, SCRIPT_NAME);
        ...
        const processingOptions: ProcessingOptionsV2 = {
            // Spread all options parsed by commander
            ...cmdOptions,
            // Explicitly ensure required fields are present and correctly typed
            openaiApiKey: cmdOptions.openaiApiKey as string, // Already checked for existence
            openaiModel: cmdOptions.openaiModel as string,
            debug: !!cmdOptions.debug, // Ensure boolean
            verbose: !!cmdOptions.verbose, // Ensure boolean
            logLevel: cmdOptions.logLevel as string,
            createPlaceholder: cmdOptions.createPlaceholder !== false, // Default is true if flag not present
        ...
        await runOrderProcessingV2(processingOptions, prisma);

    } catch (error) {
        scriptRunSuccess = false;
        const logger = getLogger(); // Get logger even if initialization failed partially
        ...
        closeLogStream(); // Close the file stream
...
main().catch((error) => {
    ...
    closeLogStream(); // Attempt to close logs even on unhandled rejection
    process.exit(1);
});
