...
import { updateOrderItemOptions, getShipstationOrders } from '../lib/shared/shipstation';
...
// Type for manual color entries
interface ManualColorEntry {
  orderId: number;
  orderNumber: string;
  items: {
    itemId: number;
...
// Type for the JSON file structure produced by find-amazon-orders-with-missing-colors.ts
interface OrderWithMissingColors {
  orderId: number;
...
function extractCustomizationUrl(printSettings: Prisma.JsonValue | null): string | null {
...
async function reprocessOrderItem(
  orderId: number,
  itemId: number,
  manualEntries: ManualColorEntry[] = []
): Promise<{
  itemId: number;
  updatedTasks: number;
  newData: {
  ...
  source: 'amazon-url' | 'manual-entry';
} | null> {
  ...
  let amazonData = null;
  let dataSource: 'amazon-url' | 'manual-entry' = 'amazon-url';

  if (customizedUrl) {
    amazonData = await fetchAndProcessAmazonCustomization(customizedUrl);
    if (amazonData) {
  ...
  for (const task of item.printTasks) {
    ...
    if (isDryRun) {
      ...
      console.log(
        `Updated task ${task.id} with colors:`,
        updateData.color_1 ? `color_1: ${updateData.color_1}` : 'color_1: unchanged',
        updateData.color_2 ? `color_2: ${updateData.color_2}` : 'color_2: unchanged'
      );
    }
    tasksUpdated++;
  }
...
async function generateTemplateFile(ordersToProcess: OrderWithMissingColors[]): Promise<void> {
  // Create template structure for manual entries
  const template: ManualColorEntry[] = [];

  for (const order of ordersToProcess) {
    const orderEntry: ManualColorEntry = {
    ...
    for (const item of order.itemsWithMissingColors) {
      ...
      if (!hasCustomizedUrl) {
        orderEntry.items.push({
          ...
          customText: null, // To be filled manually
          color1: null, // To be filled manually
  ...
  await fs.writeFile(manualEntriesFile, JSON.stringify(template, null, 2));
...
async function checkIfItemHasCustomizedUrl(itemId: number): Promise<boolean> {
  const item = await prisma.orderItem.findUnique({
    where: { id: itemId },
...
async function main() {
  const filePath = path.join(process.cwd(), 'amazon-orders-missing-colors.json');
  ...
  try {
    ...
    console.error(
      `Please run find-amazon-orders-with-missing-colors.ts first to generate the file.`
    );
    return;
  }

  // Read the file with orders to reprocess
  const ordersJson = await fs.readFile(filePath, 'utf-8');
  ...
  let manualEntries: ManualColorEntry[] = [];
  if (useManualEntries) {
    try {
      const manualEntriesJson = await fs.readFile(manualEntriesFile, 'utf8');
      manualEntries = JSON.parse(manualEntriesJson);
      console.log(`Loaded ${manualEntries.length} orders with manual color entries.`);
    } catch (error) {
      console.error(
        `Error loading manual entries file: ${error instanceof Error ? error.message : String(error)}`
      );
      console.error(
        `Make sure to run with --generate-template first and fill in the file: ${manualEntriesFile}`
      );
      process.exit(1);
    }
  }
  ...
  await fs.writeFile(resultsFilename, JSON.stringify({ results, summary }, null, 2));
