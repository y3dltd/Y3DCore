...
import { updateOrderItemOptions } from '../lib/shared/shipstation';
    ...
    for (const task of item.printTasks) {
        ...
        const updateData: Prisma.PrintOrderTaskUpdateInput = {
            annotation: task.annotation
                ...
                : (dataSource === 'amazon-url' ? 'Reprocessed with enhanced parser' : 'Updated from manual entry')
        };
        ...
        if (isDryRun) {
            ...
            console.log(`Updated task ${task.id} with colors:`,
...
main()
    .then(results => {
        const fileName = isDryRun ? 'amazon-reprocess-dryrun-results.json' : 'amazon-reprocess-results.json';
