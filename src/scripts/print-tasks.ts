        ...
        async (argv) => {
            await runCommand('Create', async () => {
                ...
                for (const order of orders) {
                    logger.info(`Processing order ${order.id} (${order.shipstation_order_number || 'No SS Number'})...`);
                        ...
                        try {
                            ...
                            const task = await prisma.printOrderTask.create({
                                data: {
                                    ...
                                    review_reason: argv.createPlaceholder ? 'Placeholder task - needs verification' : null,
                                    taskIndex: 0,
                                }
                            });
                ...
                logger.info(`Command completed: ${tasksCreated} tasks created, ${tasksSkipped} tasks skipped, ${needsReviewCount} tasks need review`);
            });
        }
    )
    .command(
                    ...
                    for (const task of tasks) {
                        if (task.orderItem?.print_settings) {
                            try {
                                ...
                                const customText = typeof settings.custom_text === 'string' ? settings.custom_text : task.custom_text;
        ...
        async (argv) => {
            await runCommand('Cleanup', async () => {
                ...
                if (argv.clearCompleted) {
                    ...
                    logger.info(`Found ${completedTasksCount} completed tasks older than ${argv.daysBack} days`);
                ...
                if (argv.fixPending) {
                    ...
                    if (argv.dryRun) {
                        ...
                        logger.info(`[DRY RUN] Would update ${pendingTasksCount} pending print tasks for shipped/cancelled orders`);
                        return;
                    }
                    ...
                    for (const order of orders) {
                        for (const task of order.printTasks) {
                            await prisma.printOrderTask.update({
                                where: { id: task.id },
                                data: {
                                    status: order.order_status === 'shipped' ? 'completed' as PrintTaskStatus : 'cancelled' as PrintTaskStatus,
                                    updated_at: new Date()
                                }
                            });
                            tasksUpdated++;
    ...
    .command(
        ...
        async (argv) => {
            await runCommand('Metrics', async () => {
                ...
                const avgProcessingTimeMs = validTasksCount > 0
                    ? totalProcessingTime / validTasksCount
                    : 0;
