...
const SPAPI_REGION = SPAPI_ENDPOINT?.includes('eu') ? 'eu-west-1' : // Infer region (adjust if needed)
                     SPAPI_ENDPOINT?.includes('na') ? 'us-east-1' :
                     SPAPI_ENDPOINT?.includes('fe') ? 'us-west-2' : 'us-east-1'; // Default or throw error

const LWA_TOKEN_ENDPOINT = 'https://api.amazon.com/auth/o2/token';

// Basic validation
if (!LWA_CLIENT_ID || !LWA_CLIENT_SECRET || !LWA_REFRESH_TOKEN || !AWS_ACCESS_KEY_ID || !AWS_SECRET_ACCESS_KEY || !SPAPI_ENDPOINT) {
...
async function getLwaAccessToken(): Promise<string> {
    ...
    try {
        const response = await axios.post(LWA_TOKEN_ENDPOINT, {
        ...
        if (error instanceof Error) {
            ...
            if (axios.isAxiosError(error)) { // Check if it's an AxiosError first
                if (error.response) { // Now safely access response
                    message = `Error refreshing LWA token: ${JSON.stringify(error.response.data)} (Status: ${error.response.status})`;
                } else { // Axios error without a response (e.g., network error)
                    message = `Error refreshing LWA token: ${error.message}`;
                }
            } else { // Not an Axios error, just use the standard Error message
                 message = `Error refreshing LWA token: ${error.message}`;
            }
        } else if (typeof error === 'string') { // Handle plain string errors
