 ...
 */
export async function syncCustomizationFiles(options: AmazonSyncOptions): Promise<AmazonSyncResult> {
    ...
    try {
        ...
        if (orderItems.length === 0) {
            logger.info('[Amazon Sync] No Amazon order items with customization URLs found for processing.');
            return result; // Return early, success is true
        }

        logger.info(`[Amazon Sync] Found ${orderItems.length} Amazon order items with customization URLs to process.`);
            ...
            try {
                ...
                if (processResult.success) {
                    ...
                    if (processResult.error) {
                        result.errors.push({ orderId: item.orderId, itemId: item.id, error: processResult.error });
                    }
                }
                ...
                result.itemsFailed++;
                result.errors.push({ orderId: item.orderId, itemId: item.id, error: `Uncaught: ${errorMessage}` });
            }
        }
 ...
 */
async function findOrderItemsToProcess(options: AmazonSyncOptions): Promise<OrderItemWithDetails[]> {
    ...
    if (options.retryFailed) {
        logger.info(`[Amazon Sync] Including failed items for retry (max ${options.maxRetries} retries)`);
        where.OR = [
            {
                amazonCustomizationFiles: { // Relation should now be recognized
                    none: {}
                }
            },
            {
        ...
        logger.info('[Amazon Sync] Processing specific order/item, not filtering by customization file status.');
    } else {
        logger.info('[Amazon Sync] Processing only items without existing customization file records.');
        where.amazonCustomizationFiles = { // Relation should now be recognized
            none: {}
        };
    }
    ...
    logger.debug(`[Amazon Sync] Found ${items.length} potential Amazon order items before filtering for CustomizedURL`);
...
function extractCustomizationUrl(item: OrderItemWithDetails): string | null {
    ...
    logger.debug(`[Amazon Sync] Could not extract CustomizedURL from item ${item.id}`, { printSettings });
    return null;
}
 ...
 */
async function processOrderItem(item: OrderItemWithDetails, options: AmazonSyncOptions): Promise<{ success: boolean; error?: string }> {
    const orderId = item.orderId;
    const itemId = item.id;
    const orderNumber = item.order.shipstation_order_number;

    logger.info(`[Amazon Sync] Processing order item ${itemId} from order ${orderNumber} (ID: ${orderId})`);
    ...
    if (!customizationUrl) {
        ...
        logger.warn(`[Amazon Sync] No CustomizedURL found for order item ${itemId} during processing step.`, {
            orderId,
            itemId,
    ...
    if (Array.isArray(item.amazonCustomizationFiles) && item.amazonCustomizationFiles.length > 0) {
        ...
        const latestFile = item.amazonCustomizationFiles.sort((a: AmazonCustomizationFile, b: AmazonCustomizationFile) => b.createdAt.getTime() - a.createdAt.getTime())[0];
        if (latestFile.processingStatus === STATUSES.PROCESSED && !options.retryFailed && !options.orderId && !options.itemId) {
    ...
    if (!customizationFile) {
        logger.error(`[Amazon Sync] Failed to find or create customization file record for item ${itemId}. Skipping.`, { orderId, itemId });
    ...
    try {
        ...
        if (!options.dryRun) {
            if (customizationFile.id !== 0) {
                logger.info(`[Amazon Sync] Updating customization file record ${customizationFile.id} for item ${itemId} as processed.`);
                ...
                logger.warn(`[Amazon Sync] Skipping DB update for item ${itemId} due to invalid customizationFile ID (0) even though not in dry run.`, { orderId, itemId });
            }
        } else {
            logger.info(`[DRY RUN] Would update customization file record for item ${itemId} as processed.`);
        }

        logger.info(`[Amazon Sync] Successfully processed customization for order item ${itemId}`, {
            orderId,
            itemId,
            extracted: { // Log only extracted fields, not raw JSON
                customText: customizationData.customText,
                color1: customizationData.color1,
                color2: customizationData.color2,
            }
        ...
        if (!options.dryRun) {
            if (customizationFile && customizationFile.id !== 0) { // Add null check for customizationFile
                logger.info(`[Amazon Sync] Updating customization file record ${customizationFile.id} for item ${itemId} as failed.`);
                // Use prisma.amazonCustomizationFile (lowercase a)
                ...
                logger.warn(`[Amazon Sync] Skipping DB error update for item ${itemId} due to invalid customizationFile ID (0) even though not in dry run.`, { orderId, itemId, error: errorMessage });
            }
        } else {
 ...
 */
async function findOrCreateCustomizationFile(orderItemId: number, url: string, options: AmazonSyncOptions): Promise<AmazonCustomizationFile | null> {
    try {
        ...
        if (!customizationFile) {
            ...
            logger.debug(`[Amazon Sync] Found existing customization file record ${customizationFile.id} for item ${orderItemId}`);
        }
        ...
        logger.error(`[Amazon Sync] Error finding or creating customization file record for item ${orderItemId}`, { error: message });
