...
import { syncSingleOrder } from '@/lib/shipstation'
import { detectMarketplaceOrderNumber, MARKETPLACE_PATTERNS } from '@/lib/order-utils'
...
async function findOrderIdByMarketplaceNumber(orderNumber: string): Promise<number | null> {
    try {
        ...
        if (order) {
            console.log(`[Server Action] Found order ID ${order.id} for marketplace order number ${orderNumber} (${order.marketplace})`)
            return order.id
        }
...
export async function runPopulateQueueForOrder(
    ...
    if (detection.isMarketplaceNumber) {
        console.log(`[Server Action] Input appears to be a ${detection.marketplace || 'unknown'} marketplace order number: ${orderStr}`)
        ...
        if (parsedId > 1000000000 && MARKETPLACE_PATTERNS.etsy.test(orderStr)) {
            ...
            if (etsyOrderId) {
                ...
                console.log(`[Server Action] Input ${orderStr} appears to be an Etsy order number, not an internal ID`)
    ...
    // Construct the command carefully using the full path to npx
    const command = `/usr/bin/npx tsx src/scripts/populate-print-queue.ts --order-id=${dbOrderId} --force-recreate`
    ...
    try {
        ...
        console.error(
            `[Server Action] Error executing command for order ${dbOrderId}:`,
            error
        )
        ...
        if (typeof error === 'object' && error !== null) {
            errorMessage = (error as { stderr?: string }).stderr || (error as Error).message || errorMessage
...
export async function syncOrderFromShipStation(
    ...
    try {
        ...
        if (detection.isMarketplaceNumber) {
            ...
            dbOrderId = order.id
            console.log(`[Server Action] Found ShipStation order ID ${shipstationOrderId} for ${order.marketplace} order ${orderStr}`)
        } else {
            // Try to parse as a direct database ID
            const parsedId = parseInt(orderStr, 10)
            if (isNaN(parsedId)) {
            ...
            if (parsedId > 1000000000 && MARKETPLACE_PATTERNS.etsy.test(orderStr)) {
                ...
                if (order) {
                    ...
                    shipstationOrderId = order.shipstation_order_id
                    dbOrderId = order.id
                    console.log(`[Server Action] Found ShipStation order ID ${shipstationOrderId} for Etsy order ${orderStr}`)
                } else {
                    // Not found as Etsy order, treat as internal ID
                    ...
                    shipstationOrderId = order.shipstation_order_id
                    dbOrderId = parsedId
                    console.log(`[Server Action] Found ShipStation order ID ${shipstationOrderId} for database ID ${dbOrderId}`)
                }
            } else {
                // Regular internal ID
                // Look up the ShipStation order ID from the database
        ...
        if (result.success) {
            ...
            console.error(`[Server Action] Failed to sync order ${shipstationOrderId} from ShipStation:`, result.error)
            return {
                success: false,
