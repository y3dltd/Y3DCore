/*
  ...
  - A unique constraint covering the columns `[shipstationLineItemKey]` on the table `OrderItem` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `OrderItem` ADD COLUMN `shipstationLineItemKey` VARCHAR(50) NULL,
    MODIFY `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    MODIFY `updated_at` TIMESTAMP NULL;
...
-- CreateIndex
CREATE UNIQUE INDEX `OrderItem_shipstationLineItemKey_key` ON `OrderItem`(`shipstationLineItemKey`);
